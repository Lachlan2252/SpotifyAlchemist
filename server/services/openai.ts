...
export async function generateCoverArt(prompt: string, style?: string): Promise<string> {
  const styleMap: Record<string, string> = {
    retro: 'retro album cover art style',
    minimalist: 'minimalist clean design',
    psychedelic: 'colorful psychedelic style',
  };
  const fullPrompt = `${style && styleMap[style] ? styleMap[style] + ' ' : ''}${prompt}`.trim();
