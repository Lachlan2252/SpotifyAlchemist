...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      for (let i = 0; i < removals.length && i < newTracks.length; i++) {
        ...
        if (oldTrack && newTrack) {
          await storage.updateTrack(oldTrack.id, {
  ...
  // Add missing user preferences endpoints
  ...
  app.put("/api/user/preferences", async (req, res) => {
    try {
      ...
      await storage.updateUserPreferences(req.session.userId, req.body);
      const updated = await storage.getUserPreferences(req.session.userId);
      res.json(updated);
    } catch (error) {
  ...
  // Add prompt suggestions endpoint
  app.get("/api/prompts/suggest", async (req, res) => {
    try {
